...
async def run_knowledge_structuring_agent(
    ...
    *,
    ...
    prompt = (
        ...
        "RULES:\n"
        "- REORGANISE, DO NOT SUMMARISE. This is filing, not compression. "
        "Every substantive line must survive into some document. If the "
        "input has little conversational filler, your output should be "
        "close to the same length as the input. Losing detail is the worst "
        "thing you can do here — it is the creator's material, and "
        "whatever you drop is gone from everything downstream.\n"
        "- Split by topic. A transcript covering a reel script, a brand "
        "name and a caption becomes three documents, not one.\n"
        "- Do not shatter a list. An inventory — past topics, episode "
        "titles, deliverables, pending items — is ONE document holding the "
        "whole list, not one document per entry. Splitting it loses the "
        "list and keeps only the headings.\n"
        "- Reference material is content, not filler. Brand style, colours "
        "and hex codes, wardrobe, settings, workflow steps, standing rules, "
        "voice settings and pending items each belong in a document of "
        "their own. They are the most reusable thing in the paste; never "
        "discard them as preamble.\n"
        ...
        "- Drop conversational filler, and nothing else. Filler means "
        "greetings, praise, acknowledgements, 'here you go', and "
        "meta-commentary about what the assistant is about to do. It never "
        "means a specific: a colour, a hex code, a name, a number, a price, "
        "a rule, a step, an item in a list. When unsure whether something "
        "is filler, keep it.\n"
